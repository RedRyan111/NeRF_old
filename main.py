...
from example_images.model import NeRF
...
#encoder = MyPositionalEncoder(10)
#viewdirs_encoder = MyPositionalEncoder(4)
...
#myencoded_points = myencoder(pts_flattened)
#print(f'original encoding: {encoded_points.shape} my encoder: {myencoded_points.shape}')

#print(torch.prod(torch.eq(encoded_points, myencoded_points)))
...
def nerf_forward(
    ...
    query_points, z_vals = sample_stratified(
        rays_o, rays_d, near, far, n_samples_hierarchical)
...
def init_models():
  ...
  """
  # Encoders
  encoder = PositionalEncoder(n_freqs)
  encode = lambda x: encoder(x)

  # View direction encoders
  if use_viewdirs:
    encoder_viewdirs = PositionalEncoder(n_freqs_views)
    encode_viewdirs = lambda x: encoder_viewdirs(x)
    d_viewdirs = encoder_viewdirs.frequency_bands.shape[0]*3*2 + 3#encoder_viewdirs.d_output
  else:
    encode_viewdirs = None
    d_viewdirs = None

  # Models
  model = NeRF(encoder.frequency_bands.shape[0]*3*2 + 3, n_layers=n_layers, d_filter=d_filter, skip=skip,
              d_viewdirs=d_viewdirs)
  model.to(device)
  model_params = list(model.parameters())
  if use_fine_model:
    fine_model = NeRF(encoder.frequency_bands.shape[0]*3*2 + 3, n_layers=n_layers, d_filter=d_filter, skip=skip,
                      d_viewdirs=d_viewdirs)
...
def train():
    ...
    for i in range(n_iters):
        ...
        if one_image_per_step:
            ...
            target_img_idx = np.random.randint(data_manager.images.shape[0])
            target_img = torch.tensor(data_manager.images[target_img_idx]).to(device)
            if center_crop and i < center_crop_iters:
                target_img = crop_center(target_img)
            height, width = target_img.shape[:2]
            target_pose = torch.tensor(data_manager.poses[target_img_idx]).to(device)
            rays = get_rays(height, width, data_manager.focal).to(device)
            rays_o, rays_d = transform_rays_with_image_orientation(rays, target_pose)
            #rays_o, rays_d = get_rays(height, width, data_manager.focal, target_pose)
        ...
        if i % display_rate == 0:
            ...
            rays = get_rays(height, width, data_manager.focal).to(device)
            rays_o, rays_d = transform_rays_with_image_orientation(rays, example_pose)
            #rays_o, rays_d = get_rays(data_manager.image_height, data_manager.image_width, data_manager.focal, example_pose)
            ...
            rgb_predicted = outputs['rgb_map']
            loss = torch.nn.functional.mse_loss(rgb_predicted, example_img.reshape(-1, 3))
            ...
            ax[1].imshow(example_img.detach().cpu().numpy())
